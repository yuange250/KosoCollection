 ...
 * 1. 解析 src/lib/project2Data.ts 中的景点元数据
 * 2. 在 zh/en Wikipedia 搜索页面
 * 3. 从页面主图和页面挂载文件中筛选景点图
 * 4. 下载到 public/images/project2/poi-*，并生成 src/lib/project2PoiCached.gen.ts
 *
 * 用法：
 *   node scripts/cache-project2-poi-images.mjs
 *   node scripts/cache-project2-poi-images.mjs --limit 5
 *   node scripts/cache-project2-poi-images.mjs --ids forbidden-city,eiffel-tower
 *   node scripts/cache-project2-poi-images.mjs --replace
 */
...
const GEN_TS = path.join(ROOT, 'src', 'lib', 'project2PoiCached.gen.ts');
const MANIFEST = path.join(OUT_DIR, 'poi-manifest.json');

const USER_AGENT = 'GameHistory/1.0 (project2 poi image crawler; educational use)';
...
async function main() {
  ...
  for (const point of points) {
    ...
    for (let index = 0; index < resolved.images.length; index += 1) {
      ...
      localPaths.push(`/images/project2/${base}`);
      imageMeta.push({
        file: base,
        source: image.source,
        title: image.title,
