/**
 * 1) 校验 project2Data：中文「分类 category」与 wmCard(kind) 是否一致（图文口径一致）。
 * 2) 按与 pickStockGalleryUrls 相同算法拉取各景点用到的 Unsplash 图到 public/images/project2/stock-*.jpg
 * 3) 生成 src/lib/project2StockCached.gen.ts，运行时空走 CDN。
 *
 * 用法: node scripts/cache-project2-unsplash.mjs
...
const DATA_TS = path.join(ROOT, 'src', 'lib', 'project2Data.ts');
const STOCK_TS = path.join(ROOT, 'src', 'lib', 'project2StockPhotos.ts');
const OUT_DIR = path.join(ROOT, 'public', 'images', 'project2');
const GEN_TS = path.join(ROOT, 'src', 'lib', 'project2StockCached.gen.ts');

const UA = 'GameHistory/1.0 (project2 unsplash cache; Unsplash License)';
...
function extractWorking(ts) {
  ...
  if (!m) throw new Error('const WORKING not found in project2StockPhotos.ts');
...
function formatGenTs(mapPointToPaths) {
  ...
  return `/**\n * 由 scripts/cache-project2-unsplash.mjs 生成 — 勿手改。重新生成：npm run cache:project2-unsplash\n */\nexport const PROJECT2_STOCK_LOCAL: Record<string, readonly string[]> = {\n${lines.join('\n')}\n};\n`;
}
    ...
    for (const url of urls) {
      ...
      if (!urlToWebPath.has(url)) {
        ...
        if (existingDisk) {
          ...
          urlToWebPath.set(url, `/images/project2/${base}`);
        } else {
