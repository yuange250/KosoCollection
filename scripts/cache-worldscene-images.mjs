/**
 * 将 src/lib/worldsceneData.ts 中的 upload.wikimedia.org 外链下载到 public/images/worldscene/
 * 并把源码里的 URL 替换为本地路径（/images/worldscene/...）。
 *
 * 用法: node scripts/cache-worldscene-images.mjs
...
const DATA_FILE = path.join(ROOT, 'src', 'lib', 'worldsceneData.ts');
...
async function main() {
  ...
  if (urls.length === 0) {
    console.log('No Wikimedia URLs found in worldsceneData.ts');
    return;
  }
  ...
  for (const url of urls) {
    ...
    const webPath = `/images/worldscene/${name}`;
